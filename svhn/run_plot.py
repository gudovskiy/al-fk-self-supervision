...
parser = argparse.ArgumentParser(description='PyTorch SVHN Run')
parser.add_argument('--dataset', default='SVHN', type=str,
                    help='dataset name')
parser.add_argument('--datadir', default='../local_data/SVHN', type=str,
                    help='dir to save/read data')
parser.add_argument('--run-start', type=int, default=0, metavar='R',
                    help='run start pointer (default: 0)')
parser.add_argument('--run-stop', type=int, default=5, metavar='R',
                    help='run stop pointer (default: 5)')
...
bSize = 1250 # batch increment
msa = [256] # descriptor lengths for multiscale aggregation
K = 64
...
#cfg.append([ir, un, 'gradAbl','topKandPccFprGrad', 768, 1])
...
#
results.append([ir, 0, 'none', 'VAAL', 'none', 1, np.load('../vaal/SVHN_ir_{}.npy'.format(ir))])
# full-dataset baseline
if   ir == 1:
    results.append([1,   0.0, 'none', 'all train', 'none', 1, np.repeat([96.20], B)])
elif ir == 100:
    results.append([100, 0.0, 'none', 'all train', 'none', 1, np.repeat([87.60], B)])
else:
    print('Wrong IR!')
    sys.exit(0)
# print table
...
for i, r in enumerate(results_list):
    if r[3] == 'VAAL':
        plt.errorbar(x, np.mean(r[6], axis=0), yerr=1*np.std(r[6], axis=0), fmt='--g', label=r'%s' % (r[3]), markersize=6, linewidth=1.5, elinewidth=1.0, capsize=4)
        j = j + 1
j = 0
for i, r in enumerate(results_list):
...
#
if   ir == 1:
    plt.axis([F, F*B, 77.0, 97.0])
    plt.yticks(np.arange(77.0, 97.0, 2.0))
elif ir == 100:
    plt.axis([F, F*B, 55.0, 90.0])
    plt.yticks(np.arange(55.0, 90.0, 3.5))
else:
    print('Wrong IR!')
    sys.exit(0)
#
if ir == 1:
    plt.annotate("",
                xy=(0.625, 86.5), xycoords='data',
                xytext=(0.625, 88.5), textcoords='data',
                arrowprops=dict(arrowstyle="<|-|>",
                connectionstyle="arc3", color='black', linewidth=0.5), size=14)
    plt.annotate("",
                xy=(1.125, 87.0), xycoords='data',
                xytext=(1.125, 90.5), textcoords='data',
                arrowprops=dict(arrowstyle="<|-|>",
                connectionstyle="arc3", color='black', linewidth=0.5), size=14)

    plt.text(0.55, 89.0, r'$1.5\%$', rotation = 0, fontsize = 12)
    plt.text(1.05, 91.0, r'$3.5\%$', rotation = 0, fontsize = 12)
    plt.text(0.3, 77.5, '(a)', rotation = 0, fontsize = 16)
elif ir == 100:
    plt.annotate("",
                xy=(0.625, 71.5), xycoords='data',
                xytext=(0.625, 80.5), textcoords='data',
                arrowprops=dict(arrowstyle="<|-|>",
                connectionstyle="arc3", color='black', linewidth=0.75), size=16)
    plt.annotate("",
                xy=(1.3, 68.0), xycoords='data',
                xytext=(1.3, 83.75), textcoords='data',
                arrowprops=dict(arrowstyle="<|-|>",
                connectionstyle="arc3", color='black', linewidth=0.75), size=16)
    plt.annotate("",
                xy=(0.75, 82.5), xycoords='data',
                xytext=(1.25, 82.5), textcoords='data',
                arrowprops=dict(arrowstyle="-|>",
                connectionstyle="arc3", color='black', linewidth=0.75), size=16)

    plt.text(0.55, 74.0, r'$10\%$', rotation = 90, fontsize = 12)
    plt.text(1.2, 74.0, r'$16\%$', rotation = 90, fontsize = 12)
    plt.text(0.85, 83.5, r'$-40\%$', rotation = 0, fontsize = 12)
    plt.text(0.55, 56.0, '(b)', rotation = 0, fontsize = 16)
#
plt.xticks(np.arange(F, F*(B+1), F))
plt.legend(loc='lower right', shadow=False, fontsize = fontLegend, ncol=1)
