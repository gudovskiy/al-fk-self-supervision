...
V = 104388 # number of validation images

def main():
    # Training settings
    parser = argparse.ArgumentParser(description='Descriptor-based Augmentation for PyTorch SVHN Example')
    ...
    parser.add_argument('-as', '--augment-size', type=int, default=1250, metavar='N',
                        help='augmentation dataset size for training (default: 1250)')
    ...
    test_loader = torch.utils.data.DataLoader(
        SVHN(args.save_folder, args.run_folder,             transform=transform_usual),
            batch_size=args.test_batch_size, shuffle=False, **kwargs)

    model = resnet10(L=args.descriptor_length).to(device)
    ...
    if args.augment_method == 'random':
        ...
        assert descr_val.size(0) == V, 'Number of val descriptors should be equal to 104128'
        ...
        S = 256 # divide MxL matrix into SxL chunks to fit into memory
    ...
    train_loader = torch.utils.data.DataLoader(
        SVHN(args.save_folder, args.run_folder, train=True, transform=transform_usual, train_list=augment_index_list,
            imbalance_ratio=args.imbalance_ratio, noisy_ratio=args.noisy_ratio),
            batch_size=args.batch_size, shuffle=True, **kwargs)

    #print('######## THIS IS AUGMENTED SVHN NETWORK RUN ########')
