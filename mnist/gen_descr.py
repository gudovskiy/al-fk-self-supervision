...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='Descriptor Generator for PyTorch MNIST Example')
    ...
    train_loader = torch.utils.data.DataLoader(
        MNIST(args.save_folder, args.run_folder, train=True, transform=transform_usual,
    ...
    if 'MC' in args.subtype_method:
        model = lenet(L=args.descriptor_length, MC=True).to(device)
    else:
        model = lenet(L=args.descriptor_length, MC=False).to(device)
    #
