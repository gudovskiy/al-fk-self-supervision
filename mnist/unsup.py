...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(
        MNIST(args.save_folder, args.run_folder, download=True, transform=transform_usual),
    ...
    if not(imbalance_ratio==1 and noisy_ratio==0.0):
        ...
        if os.path.isfile(data_file):
            ...
            if imbalance_ratio != 1:
                ...
                imbalanced_images = torch.zeros((L+S, data.size(1), data.size(2)), dtype=torch.uint8)
    ...
    train_loader = torch.utils.data.DataLoader(
        MNIST(args.save_folder, args.run_folder, train=True, download=True, transform=transform_usual,
    ...
    modelRefer = lenet(UNSUP=False).to(device)
    ...
    #
    if not os.path.isfile(checkpoint_unsup_file):
        modelUnsup = lenet(UNSUP=True).to(device)
